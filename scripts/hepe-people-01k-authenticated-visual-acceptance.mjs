...
async function inspectPage(page,id,route,viewportName){
  ...
  if(severe.length){console.error(`AXE_DETAIL:${id}:${viewportName}:${JSON.stringify(severe.map(v=>({id:v.id,impact:v.impact,nodes:v.nodes.slice(0,8).map(n=>({target:n.target,html:n.html,failureSummary:n.failureSummary}))}))}`);fail(`AXE_SERIOUS_CRITICAL:${id}:${viewportName}:${severe.map(v=>v.id).join(',')}`)}
