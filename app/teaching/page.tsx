...
async function loadTeaching(): Promise<{ state: State; rows: Row[]; note: string }> {
  ...
  if (activityResult.error || versionResult.error || deliveryResult.error || outcomeResult.error || assessmentLinkResult.error) return { state: 'QUERY_ERROR', rows: [], note: 'At least one controlled teaching source could not be read under the current RLS scope.' };
  const activities = new Map((activityResult.data ?? []).map((row) => [row.learning_activity_id, row]));
  const deliveries = new Map<string, Row[]>();for (const row of deliveryResult.data ?? []) {const key = String(row.learning_activity_version_id);deliveries.set(key, [...(deliveries.get(key) ?? []), row]);}
  const outcomes = new Map<string, number>();for (const row of outcomeResult.data ?? []) {const key = String(row.learning_activity_version_id);outcomes.set(key, (outcomes.get(key) ?? 0) + 1);}
  const assessments = new Map<string, number>();for (const row of assessmentLinkResult.data ?? []) {const key = String(row.learning_activity_version_id);assessments.set(key, (assessments.get(key) ?? 0) + 1);}
  const rows: Row[] = (versionResult.data ?? []).map((version) => {const activity = activities.get(version.learning_activity_id) ?? {};const versionId = String(version.learning_activity_version_id);const linkedDeliveries = deliveries.get(versionId) ?? [];return {activity_code: activity.activity_code ?? 'NO VERIFIED DATA',activity_type: activity.activity_type ?? 'NO VERIFIED DATA',title_th: version.title_th ?? 'NO VERIFIED DATA',version_no: version.version_no,planned_hours: version.planned_hours ?? 'NO VERIFIED DATA',clo_outcome_links: outcomes.get(versionId) ?? 0,assessment_links: assessments.get(versionId) ?? 0,recorded_deliveries: linkedDeliveries.length,latest_delivery_status: linkedDeliveries.at(-1)?.status_code ?? 'NO EVIDENCE',current_status: version.status_code};});
  return {state: rows.length ? 'VERIFIED' : 'EMPTY',rows,note: 'Read-only RLS-scoped learning activity, outcome, assessment-link and delivery context. No teaching record is fabricated when evidence is absent.'};
}
