...
async function loadPlanActual(): Promise<Result> {
  ...
  if (activityResult.error || versionResult.error || deliveryResult.error || outcomeResult.error || assessmentResult.error) return { state: 'QUERY_ERROR', rows: [], note: 'At least one controlled plan-versus-actual source could not be read under the current RLS scope.' };
  const activities = new Map((activityResult.data ?? []).map((row) => [row.learning_activity_id, row]));
  const deliveryByVersion = new Map<string, Row[]>();for (const row of deliveryResult.data ?? []) {const key = String(row.learning_activity_version_id);deliveryByVersion.set(key, [...(deliveryByVersion.get(key) ?? []), row]);}
  const outcomeCount = new Map<string, number>();for (const row of outcomeResult.data ?? []) {const key = String(row.learning_activity_version_id);outcomeCount.set(key, (outcomeCount.get(key) ?? 0) + 1);}
  const assessmentCount = new Map<string, number>();for (const row of assessmentResult.data ?? []) {const key = String(row.learning_activity_version_id);assessmentCount.set(key, (assessmentCount.get(key) ?? 0) + 1);}
  const rows: Row[] = (versionResult.data ?? []).map((version) => {const activity = activities.get(version.learning_activity_id) ?? {};const versionId = String(version.learning_activity_version_id);const deliveries = deliveryByVersion.get(versionId) ?? [];const cloLinks = outcomeCount.get(versionId) ?? 0;const assessmentLinks = assessmentCount.get(versionId) ?? 0;const deliveryRecorded = deliveries.length > 0;const classification = !deliveryRecorded ? 'NO EVIDENCE' : 'PARTIALLY ALIGNED';return {activity_code: activity.activity_code ?? 'NO VERIFIED DATA',topic_or_activity: version.title_th ?? 'NO VERIFIED DATA',planned_hours: version.planned_hours ?? 'NO VERIFIED DATA',clo_links: cloLinks,assessment_links: assessmentLinks,delivery_records: deliveries.length,latest_delivery_status: deliveries.at(-1)?.status_code ?? 'NO EVIDENCE',latest_delivery_window: deliveries.length ? `${deliveries.at(-1)?.starts_on ?? '—'} → ${deliveries.at(-1)?.ends_on ?? '—'}` : 'NO EVIDENCE',evidence_status: deliveryRecorded ? 'DELIVERY EVIDENCE PRESENT' : 'NO EVIDENCE',alignment_classification: classification};});
  return {state: rows.length ? 'VERIFIED' : 'EMPTY',rows,note: 'The pilot compares planned activity versions against recorded delivery plus controlled CLO and assessment links. It intentionally does not claim ALIGNED because complete assessment-result/evidence sufficiency and week-level teaching records are not yet available in this read model.'};
}
