...
export default async function MyCoursesPage(){
 ...
 return <main className="shell"><aside className="sidebar"><div className="brand-kicker">HEPE · TEACHER WORKSPACE</div><div className="brand">My Academic Workspace</div><p className="brand-sub">งานสอนก่อน ระบบกำกับทำงานอยู่ด้านหลัง</p><div className="env">● NON-PRODUCTION</div><nav className="nav"><a href="/">My Workspace</a><a className="active" href="/my-courses">My Courses</a><a href="/pilot-entry">Quick Entry</a><a href="/teaching">Learning & Teaching</a><a href="/assessment">Assessment</a><a href="/plan-actual">Plan vs Actual</a><a href="/evidence">Evidence</a></nav></aside><section className="workspace"><div className="module-wrap"><a className="breadcrumb" href="/">← My Academic Workspace</a><section className="module-hero"><div className="brand-kicker">HEPE · MY COURSES</div><h1>รายวิชาของฉัน</h1><p>เริ่มจากรายวิชาที่ผู้ใช้มีสิทธิ์เห็น แล้วเข้าพื้นที่ทำงานของรายวิชา บันทึกการสอน การประเมิน และติดตาม Plan vs Actual ได้จากจุดเดียว</p></section><div className="pill pass">HEPE USABLE-APP · RLS SCOPED</div><article className="card live-panel" data-tour="courses-state" style={{marginTop:14}}><div className="label">Authenticated course state</div><div className="live-state">{result.state}</div><p className="note">{result.note}</p></article>{result.rows.length?<section className="grid g2" data-tour="course-list" style={{marginTop:14}}>{result.rows.map(c=><article className="card interactive" key={c.curriculum_course_id}><div className="brand-kicker">{c.course_code}</div><h2 className="section-title">{c.title_th||c.title_en||'NO VERIFIED TITLE'}</h2><p className="note">{c.title_en&&c.title_th?c.title_en:''}{c.credit_value!==null?` · ${c.credit_value} credits`:''}</p><div className="hero-actions" style={{marginTop:12}}><a className="button" href={`/course-workspace/${c.curriculum_course_id}`}>เปิด Course Workspace</a><a className="button secondary" href="/pilot-entry">Quick Entry</a><a className="button secondary" href="/plan-actual">Plan vs Actual</a></div></article>)}</section>:null}<div className="firewall">NON-PRODUCTION / SYNTHETIC TEST DATA ONLY · RLS-scoped course visibility · Human academic authority preserved.</div></div></section></main>;
}
