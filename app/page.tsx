const modules = [
  ['tqf3','มคอ.3 Course Workspace'],['programme','Programme Overview'],['curriculum','Curriculum Structure'],['study-plan','Study Plan'],['traceability','PLO / Course Traceability'],['mapping','PLO × Course / I-R-M'],['evidence','Evidence Explorer'],['qa','QA / CPRR'],['reviews','Review Queue'],['calendar','Academic Tasks'],['audit','Provenance'],['ai','AI Advisory'],['governance','Settings / Governance']
];
...
export default function Home(){
 return <main className="shell">
  <aside className="sidebar">
   ...
   <nav className="nav sans" aria-label="HEPE modules"><a className="active" href="/">Command Center</a><a href="/tqf3">มคอ.3 Workspace</a><a href="/pilot">Controlled Pilot</a>{modules.filter(([s])=>s!=='tqf3').map(([s,l])=><a key={s} href={`/${s}`}>{l}</a>)}</nav>
  </aside>
  <section className="workspace">
   ...
   <article className="card" style={{marginBottom:14}}><div className="label">Start working</div><h2 className="section-title" style={{marginTop:8}}>มคอ.3 Course Workspace</h2><p className="note">ทดลองกรอก Course Specification, CLO/PLO, Weekly Plan, Assessment และเชื่อมไป Evidence / QA โดยข้อมูล DEMO ไม่เขียนลง canonical database.</p><a className="module-link" style={{marginTop:12}} href="/tqf3">เปิด Synthetic มคอ.3 Workspace →</a></article>
