...
function LivePanel({result}:{result:LiveResult}){
 ...
 return <article className="card live-panel" data-tour="module-live-panel" style={{marginTop:14}} aria-live="polite"><div className="label">Controlled academic binding · {result.source??'HEPE Supabase'}</div><div className="live-state">{result.state}</div><p className="note">{stateText[result.state]} No service-role bypass or fabricated fallback data is used.</p>{result.note&&<p className="note"><strong>Boundary:</strong> {result.note}</p>}{result.rows?.map((row,index)=><div className="data-record" key={index}>{Object.entries(row).map(([key,value])=><div className="row" key={key}><span>{key}</span><span className="status">{short(value)}</span></div>)}</div>)}</article>
}
