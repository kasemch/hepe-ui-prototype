...
export default async function CourseWorkspacePage({params}:{params:Promise<{id:string}>}){
  ...
  const courseQuery=`?course=${encodeURIComponent(id)}`;
  return <main className="shell"><aside className="sidebar"><div className="brand-kicker">HEPE · TEACHER WORKSPACE</div><div className="brand">Course Workspace</div><p className="brand-sub">รายวิชาเป็นจุดเริ่มต้น งานกำกับทำงานอยู่ด้านหลัง</p><div className="env">● NON-PRODUCTION</div><nav className="nav"><a href="/">My Workspace</a><a href="/my-courses">My Courses</a><a href={`/pilot-entry${courseQuery}`}>Quick Entry</a><a href={`/teaching${courseQuery}`}>Learning & Teaching</a><a href={`/assessment${courseQuery}`}>Assessment</a><a href={`/plan-actual${courseQuery}`}>Plan vs Actual</a><a href={`/evidence${courseQuery}`}>Evidence</a></nav></aside><section className="workspace"><div className="module-wrap"><a className="breadcrumb" href="/my-courses">← My Courses</a><section className="module-hero"><div className="brand-kicker">HEPE · COURSE WORKSPACE</div><h1>{c?`${c.course_code} · ${c.title_th||c.title_en||'รายวิชา'}`:'Course Workspace'}</h1><p>พื้นที่ทำงานรายวิชาเดียวสำหรับ Teaching, Assessment, Evidence และ Plan vs Actual โดยไม่ต้องไล่เมนู Governance</p></section><div className="pill pass">HEPE USABLE-APP · RLS SCOPED</div><article className="card live-panel" style={{marginTop:14}}><div className="label">Authenticated course workspace state</div><div className="live-state">{result.state}</div><p className="note">{result.note}</p></article>{c?<><section className="grid g3" style={{marginTop:14}}>{[["Learning activities",String(c.learning_activities)],["Assessments",String(c.assessments)],["Delivery records",String(c.deliveries)]].map(([a,b])=><article className="card" key={a}><div className="label">{a}</div><div className="value">{b}</div></article>)}</section><article className="card" style={{marginTop:14}}><div className="brand-kicker">COURSE CONTINUITY</div><h2 className="section-title">ทำงานรายวิชานี้ต่อ</h2><p className="note">{c.title_en??''}{c.credit_value!==null?` · ${c.credit_value} credits`:''}</p><div className="hero-actions" style={{marginTop:12}}><a className="button" href={`/pilot-entry${courseQuery}`}>บันทึกหลังสอน</a><a className="button secondary" href={`/teaching${courseQuery}`}>Teaching Record</a><a className="button secondary" href={`/assessment${courseQuery}`}>Assessment</a><a className="button secondary" href={`/plan-actual${courseQuery}`}>Plan vs Actual</a><a className="button secondary" href={`/evidence${courseQuery}`}>Evidence</a></div></article></>:null}<div className="firewall">NON-PRODUCTION / SYNTHETIC TEST DATA ONLY · Course visibility is RLS-scoped · Human academic authority preserved.</div></div></section></main>;
}
