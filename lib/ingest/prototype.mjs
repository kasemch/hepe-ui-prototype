...
export function evaluateSourceIntake(existingSources, incoming) {
  const sameHash = existingSources.find((x) => x.checksum === incoming.checksum);
  if (sameHash) return { status: 'DUPLICATE_SOURCE', existingSourceId: sameHash.sourceDocumentId };
  const sameVersion = existingSources.find((x) => x.documentVersion === incoming.documentVersion);
  if (sameVersion) return { status: 'POSSIBLE_REVISION', existingSourceId: sameVersion.sourceDocumentId };
  return { status: 'NEW_SOURCE' };
}
...
export function buildSyntheticCandidates(fixture, options = {}) {
  ...
  const extractionMethod = options.extractionMethod ?? `${sourceFormat}_STRUCTURED`;
  const candidates = [];
...
export function validatePrototype(candidates, options = {}) {
  ...
  for (const c of candidates) {
    ...
    if (c.extractionMethod === 'AI_ASSISTED_EXTRACTION') {
      push(c, 'ING-V015', 'UNVERIFIED', 'MAJOR', 'AI_ASSISTED_REQUIRES_HUMAN_VERIFICATION', undefined, 'AI-assisted extraction remains unverified', c.extractionMethod);
    }
...
export function compareParserRuns(a, b) {
  return {
    parserVersionChanged: a.parserVersion !== b.parserVersion,
    semanticChanged: JSON.stringify(a.candidates.map((c) => ({ entityType: c.entityType, businessKey: c.businessKey, fields: c.fields }))) !== JSON.stringify(b.candidates.map((c) => ({ entityType: c.entityType, businessKey: c.businessKey, fields: c.fields }))),
  };
}

export function applyHumanCorrection(reviewItem, fieldName, correctedValue, metadata) {
  return {
    ...structuredClone(reviewItem),
    correction: {
      fieldName,
      extractedValue: reviewItem.candidate.fields[fieldName],
      correctedValue,
      correctedBy: metadata.correctedBy,
      correctedAt: metadata.correctedAt,
      correctionReason: metadata.correctionReason,
      sourceRef: structuredClone(reviewItem.candidate.sourceRef),
    },
  };
}
...
export function buildReviewPackage(candidates, validations, conflicts = []) {
  ...
  return candidates.map((c) => {
    ...
    const reviewRequired = critical || issues.some((i) => ['LOW_CONFIDENCE_EXTRACTION', 'AI_ASSISTED_REQUIRES_HUMAN_VERIFICATION'].includes(i.message)) || conflictCandidates.has(c.candidateId);
    return { candidate: c, issues, reviewStatus: reviewRequired ? 'REVIEW_REQUIRED' : 'READY_FOR_HUMAN_VERIFICATION' };
  });
}

export function executeSyntheticPipeline(fixture, options = {}) {
  const sourceFormat = options.sourceFormat ?? 'XLSX';
  const parserVersion = options.parserVersion ?? '0.1.0';
  ...
  return {
    ...
    sourceFormat,
    parserVersion,
    mappingContractVersion: options.mappingContractVersion ?? 'HEPE-INGEST-01B-v0.1',
    candidates,
    validations,
    conflicts,
    review,
